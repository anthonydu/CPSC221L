...
struct BinaryTree {
 ...
 private:
  ...
  void printPaths(Node* subRoot, vector<Node*> acc) const;

  void sumDistances(const Node* subRoot, int& acc, int step) const;

  void inOrder(Node* subRoot, vector<Node*>& acc) const;
