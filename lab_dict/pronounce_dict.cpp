...
bool PronounceDict::homophones(const string& word1, const string& word2) const {
  /* Your code goes here! */
  auto upper1 = word1, upper2 = word2;
  std::transform(upper1.begin(), upper1.end(), upper1.begin(), ::toupper);
  std::transform(upper2.begin(), upper2.end(), upper2.begin(), ::toupper);
  auto found1 = dict.find(upper1);
  auto found2 = dict.find(upper2);
  if (found1 == dict.end() || found2 == dict.end()) return false;
  if (found1->second == found2->second) return true;
  return false;
}
