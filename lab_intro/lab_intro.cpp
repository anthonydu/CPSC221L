...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (pixel->r > distance * pixel->r * 0.005) {
        pixel->r -= distance * pixel->r * 0.005;
      } else {
        pixel->r = 0;
      }
      if (pixel->g > distance * pixel->g * 0.005) {
        pixel->g -= distance * pixel->g * 0.005;
      } else {
        pixel->g = 0;
      }
      if (pixel->b > distance * pixel->b * 0.005) {
        pixel->b -= distance * pixel->b * 0.005;
      } else {
        pixel->b = 0;
      }
    }
...
PNG ubcify(PNG image) {
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (colordist(*pixel, UBC_YELLOW) <= colordist(*pixel, UBC_BLUE)) {
        *pixel = UBC_YELLOW;
      } else {
        *pixel = UBC_BLUE;
      }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      if (secondPixel->r == 255) {
        if (firstPixel->r + 40 >= 255) {
          firstPixel->r = 255;
        } else {
          firstPixel->r += 40;
        }
      }
      if (secondPixel->g == 255) {
        if (firstPixel->g + 40 >= 255) {
          firstPixel->g = 255;
        } else {
          firstPixel->g += 40;
        }
      }
      if (secondPixel->b == 255) {
        if (firstPixel->b + 40 >= 255) {
          firstPixel->b = 255;
        } else {
          firstPixel->b += 40;
        }
